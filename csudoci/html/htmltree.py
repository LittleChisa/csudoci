...
html.py : API de génération de HTML en construisant la représentation
arborescente

TODO
====

*   Il faudrait encore distinguer entre les éléments qui peuvent contenir des
fils     et les éléments "terminaux" qui ne peuvent contenir aucun élément,
comme les
...
class E(object):
    ...
    def html_attrs(self, attrs=None):
        ...
        if len(attrs) == 0:
            ...
            attrs_list = [
                '{a}="{v}"'.format(a=a, v=v) for (a, v) in attrs.items()
            ]
    ...
    def html(self, indent=0, minify=False, doctype=False):
        ...
        # pour que ça fonctionne correctement, il faudrait qu'une balise
        # oneline qui se trouve dans une balise oneline ne mette pas
        # d'indentation ...
        ...
        content = cr.join(
            [child.html(indent=indent+1, minify=minify) for child in self.children]
        )
    ...
    # cette fonction reste ici pour des raisons de compatibilité avec une
    # version antérieure du package ...
